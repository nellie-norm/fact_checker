...
<style>
    ...
    h2 { font-weight: 400 !important; color: #1a1a1a !important; }
...
def query_gemini(claim: str) -> FactCheckResult:
    """Query Gemini with Google Search - only trust grounding_metadata for sources"""
    try:
        try:
            ...
            response = client.models.generate_content(
                ...
                contents=FACT_CHECK_PROMPT.format(claim=claim),
            ...
            grounded_sources = []  # Only from grounding_metadata - these are real
            
            # Extract ONLY from grounding_metadata - these are verified real URLs
                    ...
                    if hasattr(gm, 'grounding_chunks') and gm.grounding_chunks:
                        for chunk in gm.grounding_chunks:
                            if hasattr(chunk, 'web') and chunk.web:
                                ...
                                if url:
                                    grounded_sources.append(SourceEvidence(
                                        url=url,
                                        title=title or "",
                                        snippet="",
                                        supports=True
            ...
            # Parse the response but IGNORE any sources in the JSON - they're hallucinated
            result = parse_response_no_sources("Gemini", full_text)
            ...
            response = model.generate_content(FACT_CHECK_PROMPT.format(claim=claim))
            
            # No grounding available in old SDK - return with no sources
            result = parse_response_no_sources("Gemini", response.text)
            result.caveats = "No web search available. " + (result.caveats or "")
            return result
        
    except Exception as e:
        return FactCheckResult(model="Gemini", verdict="", confidence="",
                               reasoning="", error=str(e))


def parse_response_no_sources(model: str, text: str) -> FactCheckResult:
    """Parse JSON response but ignore sources (they may be hallucinated)"""
    try:
        text = text.strip()
        ...
        return FactCheckResult(
            ...
            sources=[],  # Ignore sources from JSON
...
def analyze_consensus(results: list[FactCheckResult]) -> dict:
    ...
    caveats = []
    for r in valid:
        if r.caveats and r.caveats.lower() not in ("none", "n/a", ""):
            caveats.append(f"{r.model}: {r.caveats}")
...
def fact_check_claim_streaming(claim: str, placeholders: dict):
    ...
    if os.environ.get("GOOGLE_API_KEY"):
        queries["Gemini"] = query_gemini
        render_model_card(placeholders["Gemini"], "Gemini", loading=True)
    else:
        render_model_card(placeholders["Gemini"], "Gemini", result=None)
